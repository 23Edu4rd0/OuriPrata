...
class Joia(models.Model):
    ...
    preco = models.DecimalField(max_digits=10, decimal_places=2, null=True, blank=True, help_text="Preço informativo (opcional)")
    ...
    def save(self, *args, **kwargs):
        if not self.slug:
            ...
            slug = base_slug
            contador = 1
            while Joia.objects.filter(slug=slug).exists():
                slug = f"{base_slug}-{contador}"
                contador += 1
            self.slug = slug
        super().save(*args, **kwargs)
...
class JoiaImagem(models.Model):
    joia = models.ForeignKey(Joia, on_delete=models.CASCADE, related_name='imagens_extra')
